...
B2_THEORY = {
    "mixed-conditionals": lesson(
        "Классические второй и третий типы держат причину и следствие в одном временном плане. **Смешанные условные** разводят планы: прошлое решение продолжает влиять на настоящее, или устойчивая черта объясняет уже свершившееся.\n\nСмесь нужна не «для разнообразия», а когда условие и результат живут в разных эпохах. Сначала фиксируется время условия, затем время следствия.",
        [
            rule(
    ...
    "wish-if-only": lesson(
        "**Wish** и **if only** не обещают будущее, а оценивают реальность как неудовлетворительную. Грамматический сдвиг назад маркирует нереальность: настоящее → Past Simple, прошлое → Past Perfect, чужое раздражающее поведение → would.\n\n**If only** звучит эмоциональнее, чем I wish, при той же грамматике.",
        [
            rule(
    ...
    "modals-deduction": lesson(
        "На B2 модальные часто описывают не правила, а **логический вывод** о текущем факте: от почти доказанного **must** до осторожного **might / could / may** и категорического **can't**.\n\n**Should** здесь — «ожидаемо по норме», а не совет. Контекст отделяет вывод от обязанности.",
        [
            rule(
    ...
    "past-modals": lesson(
        "Вывод или оценка о прошлом кодируется схемой **modal + have + V3**. Так оформляются почти доказанный факт (**must have**), невозможность (**can't have**), упущенная норма (**should have**) и действие, которое оказалось ненужным (**needn't have**).\n\nОтдельно стоит **didn't need to**: обязанности не было; часто действие так и не совершили.",
        [
            rule(
    ...
    "passive-perfect-infinitive": lesson(
        "Инфинитив тоже различает залог и «уже / ещё нет». **To do** — актив впереди относительно точки отсчёта. **To be done** — пассив. **To have done / to have been done** сдвигают событие раньше контрольной точки.\n\nОсобенно частотны формы после be said / thought / believed / reported и после прилагательных вроде lucky, sorry, likely.",
        [
            rule(
    ...
    "causative-have-get": lesson(
        "Каузатив нужен, когда подлежащее **не выполняет работу руками**, а устраивает её. **Have something done** звучит нейтрально-планирующе; **get something done** — энергичнее и разговорнее.\n\n**Have someone do** и **get someone to do** включают исполнителя-человека. Отдельный смысл — неприятность: I had my bag stolen.",
        [
            rule(
                "Have / get + объект + V3",
    ...
    "participle-clauses": lesson(
        "Причастный оборот упаковывает время, причину или фон без полного придаточного. **V-ing** обычно одновременно или причинно связано с главным глаголом. **V3** даёт пассив. **Having + V3** подчёркивает предшествование.\n\nПодлежащее оборота должно совпадать с подлежащим главной части — иначе получается dangling participle.",
        [
            rule(
    ...
    "inversion-intro": lesson(
        "Ограничительные и отрицательные наречия в начале предложения вызывают **инверсию**: вспомогательный глагол встаёт перед подлежащим, как в вопросе, но это утверждение.\n\nНа B2 достаточно частотного набора: never, rarely, seldom, not only… but also, hardly / scarcely… when, no sooner… than, only then / only after. Полный список — на C1.",
        [
            rule(
    ...
    "cleft-sentences": lesson(
        "**Cleft** перестраивает простое предложение так, чтобы одна единица стала ремой — новой или контрастной информацией. **It-cleft**: It was the intern who spotted the mismatch. **Wh-cleft**: What we need is a quieter room. **All-cleft** сужает до единственного желаемого: All I asked for was a receipt.\n\nЭто инструмент устной эмфазы и аккуратного письменного контраста.",
        [
            rule(
    ...
    "narrative-tenses": lesson(
        "Рассказ держит несколько слоёв: скелет событий (**Past Simple**), фон (**Past Continuous**), «ещё раньше» (**Past Perfect**), длительность до точки (**Past Perfect Continuous**).\n\n**Used to** — прекратившаяся привычка или состояние; **would** — только повторяющиеся действия в уже заданном прошлом, не состояния вроде live / know.",
        [
            rule(
    ...
    "unreal-past": lesson(
        "Ряд конструкций берёт Past Simple или Past Perfect не потому что событие было вчера, а потому что оно **гипотетично, желательно или контрфактично**. Это тот же сдвиг для нереальности, что в wish и условных.\n\n**It's time** + Past подталкивает к действию. **I'd rather** + Past — предпочтение о другом человеке. **As if / as though** выбирают Simple или Perfect по тому, насколько ситуация ложна.",
        [
            rule(
    ...
    "relative-advanced": lesson(
        "Относительное придаточное либо **сужает класс** (defining: без запятых, that допустим), либо **добавляет комментарий** к уже известному (non-defining: запятые, that нельзя).\n\nНа B2 к who / which / that добавляются whose, where / when, конструкции предлог + which / whom и сокращения до причастия.",
        [
            rule(
